...
export default function ExercicesCorpsHumain() {
  ...
  // --- ÉTATS POUR LE LOGIN ET L'ENVOI ---
  ...
  const GOOGLE_WEB_APP_URL = "src/1eAnnee/science/corps copy.jsx"; 
  ...
  // On mélange les mots au chargement de l'application
  useEffect(() => {
    setShuffledMotsEx1(shuffleArray(data.exercice1_appariement.map(d => d.nom)));
    setShuffledMotsEx2(shuffleArray(data.exercice2_glisser_deposer.map(d => d.nom)));
  }, []);

  // --- Gestion de l'Audio (Texte vers Voix) ---
  const playAudio = (texte) => {
    if ('speechSynthesis' in window) {
      window.speechSynthesis.cancel(); // Coupe la voix en cours
  ...
  const handleRetry = () => {
    ...
    if (currentStep === 1) { 
      ...
      setShuffledMotsEx1(shuffleArray(data.exercice1_appariement.map(d => d.nom))); // Remélange !
    }
    else if (currentStep === 2) {
      setDroppedEx2({});
      setShuffledMotsEx2(shuffleArray(data.exercice2_glisser_deposer.map(d => d.nom))); // Remélange !
    }
    ...
    <div className="exercice-container pop-in">
      ...
      <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '40px' }}>
        {shuffledMotsEx2.map((mot, idx) => (
          <div key={`drag-ex2-${idx}`} draggable onDragStart={(e) => handleDragStart(e, mot)} className="drag-item">
            {mot}
          </div>
        ))}
      </div>
