...
export default function ExercicesCorpsHumain() {
  ...
  // États pour gérer la pause entre les exercices et les scores
  ...
  // --- NOUVEAUX ÉTATS POUR LE LOGIN ET L'ENVOI ---
  const [studentName, setStudentName] = useState("");
  const [studentClass, setStudentClass] = useState("");
  const [isLoggedIn, setIsLoggedIn] = useState(false);
  const [isSending, setIsSending] = useState(false);
  
  // ⚠️ REMPLACE CECI PAR L'URL DE TON GOOGLE APPS SCRIPT
  const GOOGLE_WEB_APP_URL = "https://script.google.com/macros/s/AKfycbyhkIBraknfEP3LdYNiacUIYlz2Lx3rVGicDtmv-UfsZtk20R11Qg9kw-GujX7MclDBQw/exec"; 
  ...
  // --- Gestion de l'Audio (Texte vers Voix) ---
  const playAudio = (texte) => {
    if ('speechSynthesis' in window) {
      const msg = new SpeechSynthesisUtterance();
      msg.text = texte;
      msg.lang = 'fr-FR'; // Définit la langue en français
  ...
  // --- Calcul du score maximum par étape ---
  const getMaxScoreForStep = (step) => {
    if (step === 1) return data.exercice1_appariement.length;
    if (step === 2) return data.exercice2_glisser_deposer.length;
    if (step === 3) return data.exercice3_schema.zones.length;
    if (step === 4) return data.exercice4_qcm.length;
    return 0;
  };
  ...
  const handleValidate = () => {
    ...
    if (currentStep === 1) {
      matchedPairs.forEach(p => {
        if (data.exercice1_appariement[p.imgIdx].nom === p.wordNom) stepScore++;
      });
    } else if (currentStep === 2) {
      data.exercice2_glisser_deposer.forEach((item, idx) => {
        if (droppedEx2[idx] === item.nom) stepScore++;
      });
    } else if (currentStep === 3) {
      data.exercice3_schema.zones.forEach((z, idx) => {
        if (droppedImage[idx] === z.attendu) stepScore++;
      });
    } else if (currentStep === 4) {
      data.exercice4_qcm.forEach((q, idx) => {
    ...
    setStepResultVisible(true); // Affiche le panneau de résultat intermédiaire
  };

  // --- Recommencer l'exercice en cours ---
  const handleRetry = () => {
    setShowFeedback(false);
    setStepResultVisible(false);
    
    // Réinitialise uniquement les réponses de l'exercice actuel
    if (currentStep === 1) { setMatchedPairs([]); setSelectedEmojiIdx(null); }
    else if (currentStep === 2) setDroppedEx2({});
    else if (currentStep === 3) setDroppedImage({});
    else if (currentStep === 4) setMcqAnswers({});
  };

  // --- Passer à l'exercice suivant ---
  const handleNextStep = () => {
    // Sauvegarde le score de cet exercice
  ...
  const renderLogin = () => (
    <div className="exercice-container pop-in" style={{ textAlign: 'center', padding: '50px 20px' }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column', gap: '20px', maxWidth: '300px', margin: '0 auto' }}>
        ...
        <input 
          type="text" 
          placeholder="Ta classe (ex: CM1, 6ème B...)" 
          value={studentClass} 
  ...
  const renderExercice1 = () => {
    const motsMelanges = [...data.exercice1_appariement].reverse();
        ...
        <Xwrapper>
          <div style={{ display: 'flex', justifyContent: 'space-around', position: 'relative', padding: '20px' }}>
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {data.exercice1_appariement.map((m, idx) => (
                <button key={`emoji-${idx}`} id={`emoji-${idx}`} 
            ...
            <div style={{ display: 'flex', flexDirection: 'column', gap: '20px' }}>
              {motsMelanges.map((b, idx) => (
                <button key={`word-${idx}`} id={`word-${idx}`} 
                  ...
                  onClick={() => {
                    if (!showFeedback && selectedEmojiIdx !== null) {
                      ...
                      setMatchedPairs([...newPairs, { imgIdx: selectedEmojiIdx, wordNom: b.nom, wordId: `word-${idx}` }]);
            ...
            {matchedPairs.map((p, i) => {
              const isCorrect = data.exercice1_appariement[p.imgIdx].nom === p.wordNom;
              const lineColor = showFeedback ? (isCorrect ? '#48BB78' : '#F56565') : '#A0C4FF';
              return <Xarrow key={i} start={`emoji-${p.imgIdx}`} end={p.wordId} color={lineColor} strokeWidth={5} path="smooth" showHead={true} />;
            })}
          </div>
        </Xwrapper>
      </div>
  ...
  const renderExercice2 = () => (
    <div className="exercice-container pop-in">
      ...
      <div style={{ display: 'flex', gap: '12px', justifyContent: 'center', flexWrap: 'wrap', marginBottom: '40px' }}>
        {data.exercice2_glisser_deposer.map((item, idx) => (
          <div key={`drag-ex2-${idx}`} draggable onDragStart={(e) => handleDragStart(e, item.nom)} className="drag-item">
            {item.nom}
          </div>
        ))}
      </div>
  ...
  const renderExercice3 = () => (
    <div className="exercice-container pop-in">
      ...
      <p className="subtitle">Glisse les mots pour identifier toutes les parties du corps.</p>

      <div style={{ display: 'flex', gap: '12px', flexWrap: 'wrap', justifyContent: 'center', marginBottom: '40px' }}>
        {data.exercice3_schema.etiquettes_a_glisser.map((nom, idx) => (
          <div key={`drag-ex3-${idx}`} draggable onDragStart={(e) => handleDragStart(e, nom)} className="drag-item">
            {nom}
          </div>
        ))}
      </div>

      <div style={{ position: 'relative', width: '100%', maxWidth: '500px', margin: '0 auto', filter: 'drop-shadow(0 10px 20px rgba(0,0,0,0.1))' }}>
        <img src="/corps.png" alt="Le corps humain" style={{ width: '100%', borderRadius: '20px', border: '4px solid #FFF' }} />
        
        {data.exercice3_schema.zones.map((z, idx) => {
          ...
          <div key={`qcm-${idx}`} className="mcq-card">
            ...
            <div style={{display: 'flex', gap: '10px', justifyContent: 'center', flexWrap: 'wrap'}}>
              {q.options.map((opt, optIdx) => {
                ...
                return (
                  <button key={`opt-${idx}-${optIdx}`} onClick={() => !showFeedback && setMcqAnswers({...mcqAnswers, [idx]: opt})} className={btnClass}>
                    {opt}
                  </button>
                );
              })}
  ...
  const sendResultsToGoogle = async (totalScoreFinal, maxScore) => {
    ...
    const payload = {
      subject: "Sciences", // Identifiant pour le Google Script
      story: "Le Corps Humain",
      studentName: studentName,
      studentClass: studentClass,
      score: totalScoreFinal,
      total: maxScore,
      scoresDetails: scores // Envoie {1: scoreEx1, 2: scoreEx2, 3: scoreEx3, 4: scoreEx4}
    };
    ...
    return (
      <div className="exercice-container pop-in" style={{ textAlign: 'center', padding: '60px 20px', background: 'linear-gradient(135deg, #fdfbfb 0%, #ebedee 100%)' }}>
        ...
        <div style={{display: 'flex', gap: '20px', justifyContent: 'center', marginTop: '30px', flexWrap: 'wrap'}}>
          ...
          >
            {isSending ? "Envoi en cours..." : "📤 Envoyer mon score"}
          </button>
  ...
  return (
    <div className="app-main-container">
      ...
      {/* AFFICHE SOIT LE LOGIN, SOIT LES EXERCICES */}
      {!isLoggedIn ? (
        renderLogin()
      ) : (
        <>
          ...
          {/* PANNEAU DE VALIDATION / RÉSULTAT INTERMÉDIAIRE */}
          {currentStep <= 4 && (
