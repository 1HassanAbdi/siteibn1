...
export default function QuizOQREM3a() {
  ...
  // ✨ RÉCUPÉRATION DYNAMIQUE DES THÈMES DEPUIS LE JSON
    ...
    const envois = themesKeys.map(key => {
      ...
      return fetch(GOOGLE_SCRIPT_URL, {
        ...
        body: JSON.stringify({
          ...
          eleve: nom,
          date: dateStr,
          matiere: data.matiere, // ✨ Récupéré du JSON (MATHEMATIQUES ou FRANCAIS)
          domaine: key,
  ...
  if (etape === "login") {
    // Design du login (inchangé mais prêt pour le dynamique)
    return (
      <div className="min-h-screen bg-gradient-to-br from-amber-100 to-orange-200 p-8 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-2xl p-8 border-t-8 border-amber-500">
          <h1 className="text-3xl font-bold text-center mb-6">{data.matiere}</h1>
  ...
  if (resultats) {
    ...
    return (
      <div className="min-h-screen bg-green-50 p-8 flex flex-col items-center justify-center">
        ...
        <h1 className="text-3xl font-bold mb-6 text-center">Résultats - {nom}</h1>
        ...
        {envoiEnCours && <div className="mt-4 flex items-center gap-2 text-blue-500"><Loader2 className="animate-spin" /> Synchro Cloud...</div>}
      </div>
    );
  }
  ...
  return (
    <div className="min-h-screen bg-slate-50 p-4 md:p-8">
      <div className="max-w-4xl mx-auto bg-white p-6 rounded-2xl shadow-xl border-t-8 border-blue-600">
        ...
        {/* ✨ NAVIGATION AUTOMATIQUE : se base sur le nombre de thèmes dans le JSON */}
        ...
        <div className="space-y-8">
          <div className="flex justify-between items-center border-b pb-2">
            <h2 className="text-2xl font-black text-indigo-900">{themesKeys[partieActive - 1]}</h2>
            <span className="bg-indigo-100 text-indigo-700 px-3 py-1 rounded-full font-bold">
              Partie {partieActive} / {totalThemes}
            </span>
          </div>
        ...
        {/* Boutons Suivant / Précédent automatiques */}
        <div className="mt-12 flex justify-between border-t pt-6">
          ...
          {partieActive < totalThemes ? (
            <button 
              ...
              className={`px-10 py-4 rounded-xl font-black text-white ${partieComplete(partieActive) ? "bg-blue-600 hover:bg-blue-700 shadow-lg" : "bg-gray-300"}`}
            >
              THÈME SUIVANT
            </button>
          ) : (
            <button 
              ...
              className={`px-10 py-4 rounded-xl font-black text-white ${partieComplete(totalThemes) ? "bg-indigo-600 hover:bg-indigo-700 shadow-lg" : "bg-gray-300"}`}
            >
              TERMINER LE QUIZ
            </button>
          )}
