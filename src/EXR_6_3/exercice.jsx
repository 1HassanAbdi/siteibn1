...
export default function PortailOQRE({ exerciseSlug, level, onBack }) {
  ...
  const toggleSpeak = (text, id) => {
    if (currentlyReadingId === id) {
      ...
      window.speechSynthesis.cancel();
      const utterance = new SpeechSynthesisUtterance(text.replace(/\|/g, ' ')); // On enlève les barres pour l'audio
    ...
    <div className="h-screen w-screen bg-slate-950 flex flex-col overflow-hidden border-[10px] border-black">
      <header className="h-14 bg-black flex items-center justify-between px-8 border-b border-slate-900">
        <div className="flex flex-col text-left">
            <span className="text-white font-black text-[10px] uppercase">{studentInfo.nom} | {studentInfo.classe}</span>
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        ...
        <div className="w-full lg:w-1/2 bg-slate-900 p-8 overflow-y-auto border-r border-black custom-scrollbar">
          ...
          {currentPart?.sections[0]?.image && (
            <img src={currentPart.sections[0].image} className="w-full h-auto rounded-xl mb-6 border-4 border-black shadow-xl" alt="Illustration" />
          )}

          {currentPart?.sections[0]?.texte_integral?.map((t, i) => {
            const pid = `para-${i}`;
            return (
              <div key={i} className="group relative bg-slate-900/50 hover:bg-slate-800/30 p-5 rounded-2xl transition-all mb-4 border border-white/5">
                <p className="text-slate-300 text-lg leading-relaxed pr-10">
                    ...
                    {/* LOGIQUE DE RETOUR À LA LIGNE AVEC | */}
                    {t.contenu.split('|').map((ligne, idx) => (
                      <React.Fragment key={idx}>
                        {ligne.trim()}
                        {idx < t.contenu.split('|').length - 1 && <br />}
                      </React.Fragment>
                    ))}
                </p>
                <button onClick={() => toggleSpeak(t.contenu, pid)}
                  className={`absolute top-5 right-4 p-2 rounded-xl transition-all ${currentlyReadingId === pid ? 'bg-red-600 text-white' : 'bg-slate-800 text-slate-400 opacity-0 group-hover:opacity-100 hover:bg-indigo-600 hover:text-white'}`}>
        ...
        <div className="w-full lg:w-1/2 bg-slate-800 flex flex-col relative">
          <div className="flex-1 p-8 flex items-center justify-center">
            <AnimatePresence mode="wait">
              <motion.div key={currentQuestion?.numero} initial={{ opacity: 0, x: 30 }} animate={{ opacity: 1, x: 0 }} exit={{ opacity: 0, x: -30 }} className="w-full max-w-xl bg-slate-900 p-10 rounded-[3rem] border-4 border-black relative shadow-2xl">
                <div className="absolute -top-5 -left-5 bg-indigo-600 text-white w-12 h-12 flex items-center justify-center rounded-2xl font-black border-4 border-black text-xl shadow-lg">{currentQuestion?.numero}</div>
                ...
                {currentQuestion?.options ? (
                    <div className="grid gap-3">
                        {currentQuestion.options.map((opt, i) => (
                            <button key={i} onClick={() => setAnswers({...answers, [currentQuestion.numero]: opt})} className={`w-full p-5 rounded-2xl text-left font-bold border-2 transition-all shadow-md active:scale-95 ${answers[currentQuestion.numero] === opt ? "bg-indigo-600 border-white text-white translate-x-2" : "bg-slate-800 border-black text-slate-400 hover:bg-slate-700"}`}>{opt}</button>
                        ))}
                    </div>
                ) : (
                    <textarea value={answers[currentQuestion?.numero] || ""} onChange={e => setAnswers({...answers, [currentQuestion.numero]: e.target.value})} className="w-full h-48 bg-black text-white p-5 rounded-3xl outline-none border-2 border-slate-800 focus:border-indigo-600 text-lg" placeholder="Tape ta réponse ici..." />
                )}
              </motion.div>
            </AnimatePresence>
          </div>

          {/* BARRE DE NAVIGATION QUIZ */}
          <div className="p-6 bg-black flex items-center gap-6">
            <button disabled={currentQuestionIdx === 0} onClick={() => setCurrentQuestionIdx(i => i-1)} className="flex-1 py-4 bg-slate-900 text-slate-500 rounded-2xl font-black uppercase text-xs hover:bg-slate-800 disabled:opacity-20 transition-all">Retour</button>
            <div className="relative w-14 h-14 flex items-center justify-center shrink-0">
                <svg className="w-full h-full transform -rotate-90">
                    <circle cx="28" cy="28" r="22" stroke="#1e293b" strokeWidth="4" fill="transparent" />
                    <circle cx="28" cy="28" r="22" stroke="#4f46e5" strokeWidth="4" fill="transparent" strokeDasharray={138} strokeDashoffset={138 - (progress/100)*138} strokeLinecap="round" className="transition-all duration-700" />
                </svg>
            ...
            {currentQuestionIdx === currentQuestions.length - 1 ? (
              <button onClick={submitPart} disabled={!isPartComplete || isSubmitting} className={`flex-[2] py-4 rounded-2xl font-black text-xs uppercase flex items-center justify-center gap-2 transition-all ${isPartComplete ? 'bg-emerald-600 text-white shadow-lg' : 'bg-slate-800 text-slate-600 cursor-not-allowed'}`}>
                {isSubmitting ? <Loader2 className="animate-spin"/> : isPartComplete ? <><Send size={16}/> Envoyer le test</> : <><AlertCircle size={16}/> Incomplet</>}
              </button>
            ) : (
              <button onClick={() => setCurrentQuestionIdx(i => i+1)} className="flex-[2] py-4 bg-indigo-600 text-white rounded-2xl font-black uppercase text-xs hover:bg-indigo-500 shadow-lg active:scale-95 transition-all">Suivant</button>
            )}
          </div>
        </div>
      </div>
...
function LoginView({ studentInfo, setStudentInfo, handleLogin, onBack }) {
  return (
    <div className="h-screen bg-black flex items-center justify-center p-4 border-[10px] border-slate-900">
      <motion.form initial={{ opacity: 0, y: 20 }} animate={{ opacity: 1, y: 0 }} onSubmit={handleLogin} className="bg-slate-900 p-10 rounded-[3rem] border-4 border-black max-w-sm w-full shadow-2xl">
        ...
        <h1 className="text-xl font-black text-white text-center mb-8 uppercase italic tracking-tight text-indigo-400">Portail Élève</h1>
        <div className="space-y-4">
          ...
          <button className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase italic shadow-lg active:scale-95 transition-all">Valider</button>
          <button type="button" onClick={onBack} className="w-full text-slate-600 font-bold text-[10px] uppercase mt-2 hover:text-slate-400 transition-all">Quitter</button>
        </div>
      </motion.form>
    </div>
  );
    ...
    return (
      <div className="h-screen bg-slate-950 flex items-center justify-center p-6 text-center border-[10px] border-black">
        <div className="bg-slate-900 p-12 rounded-[3.5rem] border-4 border-black max-w-sm w-full shadow-2xl">
          ...
          <h2 className="text-white font-black text-2xl uppercase mb-8 italic tracking-tighter">{part?.id} Terminée</h2>
          <div className="bg-black p-8 rounded-[2.5rem] border-2 border-slate-800 mb-10 text-white text-6xl font-black shadow-inner">
            {score.score}<span className="text-xl text-slate-700">/{score.total}</span>
          </div>
          <button onClick={onNext} className="w-full bg-blue-600 text-white py-5 rounded-2xl font-black uppercase mb-4 active:scale-95 transition-all shadow-lg">{isLast ? "Terminer" : "Partie Suivante"}</button>
          <button onClick={onStop} className="w-full text-slate-500 font-black text-[10px] uppercase hover:text-white transition-all">Accueil</button>
        </div>
      </div>
    );
}
...
function FinishView({ nom, onHome }) {
    return (
      <div className="h-screen bg-indigo-600 flex flex-col items-center justify-center p-6 text-center border-[12px] border-black text-white">
        <Trophy size={100} className="mb-8 text-white/20 animate-bounce" />
        <div className="bg-white p-12 rounded-[4rem] border-8 border-black max-w-md w-full shadow-[25px_25px_0_rgba(0,0,0,1)] text-black">
          <h2 className="text-4xl font-black uppercase italic mb-6">Bravo {nom} !</h2>
          <p className="font-bold text-slate-600 mb-10 italic">Tes réponses ont été envoyées avec succès à ton enseignant.</p>
          <button onClick={onHome} className="bg-black text-white px-12 py-5 rounded-2xl font-black text-xs uppercase italic active:scale-95 transition-all shadow-xl">Retour à l'accueil</button>
        </div>
      </div>
    );
}