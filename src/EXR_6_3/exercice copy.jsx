...
export default function PortailOQRE() {
 ...
 // const currentQuestions = useMemo(() => currentPart?.sections.flatMap(s => s.questions) || [], [activePartIdx, selectedSession]);
  ...
  // 1. On récupère les questions dans l'ordre original
  const questions = currentPart?.sections.flatMap(s => s.questions) || [];

  // 2. On crée une version où SEULES les options sont mélangées
  return questions.map(q => {
    if (q.options) {
      return {
        ...q,
        options: shuffleArray(q.options) // Mélange des choix A, B, C, D
      };
    }
    return q; // Laisse tel quel si c'est une question ouverte
  });
}, [activePartIdx, selectedSession]);
        ...
        <div className="min-h-screen bg-slate-950 text-white font-sans border-[10px] border-black overflow-x-hidden">
          ...
          <section className="py-20 px-6 max-w-6xl mx-auto">
            ...
            <div className="grid md:grid-cols-2 gap-10">
                <div onClick={() => onSelect( billet)} className="cursor-pointer group bg-slate-900 p-10 rounded-[3rem] border-4 border-black hover:border-blue-600 transition-all shadow-xl">
                    <h3 className="text-3xl font-black italic mb-2 uppercase text-blue-500">LE BILLET DE CENT DOLLARS </h3>
                ...
                <div onClick={() => onSelect(Lapieuvre)} className="cursor-pointer group bg-slate-900 p-10 rounded-[3rem] border-4 border-black hover:border-blue-600 transition-all shadow-xl">
                    <h3 className="text-3xl font-black italic mb-2 uppercase text-orange-500">La pieuvre</h3>
