...
// --- COMPOSANT DE RENDU DES OPTIONS ---
...
export default function QuizOQRE3e6e() {
  ...
  const chargerData = useCallback(async () => {
    ...
    try {
      const path = `./${fichier}`;
      if (!jsonModules[path]) throw new Error(`Fichier ${fichier} introuvable.`);
      const module = await jsonModules[path]();
      setData(module.default);
      return module.default;
    } catch (err) {
      setError("Erreur : Fichier JSON introuvable.");
      return null;
    } finally {
      setChargement(false);
    }
      ...
      <div className="min-h-screen bg-amber-50 p-8 flex items-center justify-center">
        <div className="max-w-md w-full bg-white rounded-3xl shadow-xl p-10 border-t-8 border-amber-500">
          ...
          <input 
            ...
            className={`w-full p-4 border-2 rounded-xl mb-6 ${error && !email.endsWith('@eibschool.ca') ? 'border-red-500' : ''}`} 
          />
          ...
          <button
            onClick={async () => {
              ...
              if (!nom.trim() || !niveau || !fichier || !email.trim()) {
                setError("Veuillez remplir tous les champs.");
                return;
              }
              
              // 2. Vérification stricte du domaine de l'email
              if (!email.toLowerCase().endsWith("@eibschool.ca")) {
                setError("Veuillez utiliser votre courriel de l'école (@eibschool.ca).");
                return;
              }
  ...
  return (
    <div className="min-h-screen bg-slate-100 p-4">
      <div className="max-w-4xl mx-auto bg-white rounded-[32px] shadow-2xl overflow-hidden">
        ...
        <div className="p-8">
          <h2 className="text-3xl font-extrabold mb-8 pl-5 py-4 border-l-8 border-blue-600 bg-gradient-to-r from-blue-100 via-indigo-50 to-cyan-100 rounded-r-2xl shadow-lg text-blue-900 tracking-wide">
  📚 {data.themes[themeActuel]?.domaine}
</h2>

<h3 className="text-4xl font-black mb-8 px-6 py-5 bg-gradient-to-r from-indigo-600 via-blue-600 to-cyan-500 text-white rounded-2xl shadow-xl relative overflow-hidden">
  <span className="absolute inset-0 bg-white/10 backdrop-blur-sm"></span>
  <span className="relative z-10 flex items-center gap-3">
    ✨ {data.texte_integral[0]}
  </span>
</h3>
